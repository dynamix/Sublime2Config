...
def get_twin_path(path):
  ...
  if spec_file:
    ...
    if path.find("/lib/") > 0:
      return path.replace("/lib/", "/spec/").replace(".rb", "_spec.rb")
    else:
...
class RunSpecsCommand(sublime_plugin.TextCommand):
  def run(self, edit):
    ...
    self.run_in_terminal('bundle exec spec ' + path)

  def run_in_terminal(self, command):
    osascript_command = 'rvm 1.9.3-p125 do ruby '
    osascript_command += '"' + sublime.packages_path() + '/User/iterm_action.rb"'
    osascript_command += ' "' + command + '"'
    osascript_command += ' "Specs"'
    #sublime.message_dialog(osascript_command)
